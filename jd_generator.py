...
class JobDescriptionGenerator:
    ...
    def __init__(self, db_path: str = None, sample_limits: dict = None):
        ...
        self._summary_matcher = None
        self._resp_matcher = None
        self._offers_matcher = None
    ...
    def _build_category_corpus(self) -> pd.DataFrame:
        norm = {
            k.lower().replace("_", " "): v
            for k, v in self.skills_db.items()
        }
    ...
    def category_matcher(self) -> ResumeMatcher:
        if self._category_matcher is None:
            ...
            self._category_matcher.fit(
                self._build_category_corpus(),
                text_col="Resume"
            )
    ...
    def summary_matcher(self) -> ResumeMatcher:
        if self._summary_matcher is None:
            ...
            self._summary_matcher.fit(
                self._build_section_corpus(self.summaries),
                text_col="Resume"
            )
    ...
    def resp_matcher(self) -> ResumeMatcher:
        if self._resp_matcher is None:
            ...
            self._resp_matcher.fit(
                self._build_section_corpus(self.responsibilities),
                text_col="Resume"
            )
    ...
    def offers_matcher(self) -> ResumeMatcher:
        if self._offers_matcher is None:
            ...
            self._offers_matcher.fit(
                self._build_section_corpus(self.offers),
                text_col="Resume"
            )
    ...
    def _fetch_summary(self, resume_text: str, category: str) -> str:
        ...
        result = self.summary_matcher.match(
            resume_text,
            top_n=1
        )
    ...
    def _fetch_responsibilities(
        ...
        if category in self.responsibilities:
            ...
            result = self.resp_matcher.match(
                resume_text,
                top_n=1
            )
    ...
    def _fetch_offers(
        ...
        if category in self.offers:
            ...
            result = self.offers_matcher.match(
                resume_text,
                top_n=1
            )
    ...
    def _sample(self, items: list, section: str) -> list:
        ...
        indices = sorted(
            random.sample(range(len(items)), k)
        )
    ...
    def _generate_rule_based(
        ...
        summary_tpl = self._fetch_summary(
            resume_text,
            category
        )

        resp_lines = self._fetch_responsibilities(
            resume_text,
            category
        )

        offer_lines = self._fetch_offers(
            resume_text,
            category
        )
        ...
        req_lines = [
            f"  - {years}+ years of hands-on professional experience"
        ]

        for domain, domain_skills in skill_groups.items():
            req_lines.append(
                f"  - {domain}: {', '.join(domain_skills)}"
            )
