...
""")
    ...
    if st.button("Classify", type="primary", use_container_width=True):
        if len(text_in.strip()) < 20:
            ...
            with col_a:
                ...
                st.markdown(f'<span class="badge">🏷 {result["category"]}</span>', unsafe_allow_html=True)
    ...
    cat_filter = col2.selectbox("Filter by category (optional)", ["All"] + sorted(df["Category"].unique().tolist()))
    ...
    if st.button("Find matches", type="primary", use_container_width=True):
        if len(jd_in.strip()) < 10:
            ...
            for _, row in results.iterrows():
                with st.expander(f"#{int(row['rank'])} {row['Category']} — {row['similarity_pct']}% similar"):
    ...
    resume_in = st.text_area("Resume text", height=200)
    ...
    with tab1:
        ...
        sns.barplot(x=dist.values, y=dist.index, ax=ax)
        st.pyplot(fig)
    ...
    with tab2:
        ...
        axes[0].hist(df["char_len"], bins=40)
        axes[1].hist(df["word_count"], bins=40)
        st.pyplot(fig)
